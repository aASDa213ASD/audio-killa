...
public:
    ...
    void getDevices() {
        ...
        for (UINT i = 0; i < count; i++) {
            ...
            device_name = pDevice->OpenPropertyStore(STGM_READ, &pProps);
            PropVariantInit(&varName);
            device_name = pProps->GetValue(PKEY_Device_FriendlyName, &varName);
    ...
    void killDevice(int deviceID) {
        if (deviceID == NULL) {
            printf("No ID provided.");
            return;
        }

        device_hr = pEnumerator->EnumAudioEndpoints(eCapture, DEVICE_STATE_ACTIVE, &pCollection);

        UINT count;
        device_hr = pCollection->GetCount(&count);

        for (UINT i = 0; i < count; i++) {
            IMMDevice* pDevice = NULL;
            device_hr = pCollection->Item(i, &pDevice);
            device_hr = pDevice->GetId(&pwszID);
            device_hr = pEnumerator->GetDevice(pwszID, &pDevice);
            device_hr = pDevice->Activate(__uuidof(IAudioEndpointVolume),
                CLSCTX_ALL, NULL, (void**)&pEndpointVolume);
            device_hr = pEndpointVolume->GetMute(&bMute);
            ...
            if (i == deviceID) {
                if (!bMute) {
                    device_hr = pEndpointVolume->SetMute(TRUE, NULL);
                    wprintf(L"Muted device %s (ID %d)\n", varName.pwszVal, i);
                    break;
                }
                else {
                    device_hr = pEndpointVolume->SetMute(FALSE, NULL);
